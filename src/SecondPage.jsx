...
export default function SecondPage({ selectedActivities = [], selectedOrigin = {}, onBack }) {
  ...
  const isKathmanduActivity = (activityName) => {
    ...
    const kathmanduKeywords = [
      ...
      // Add all Kathmandu activities from asset folders
      'shopping', 'massage', 'spa', 'hiking', 'pottery', 'meditation', 'canyoning'
    ];
  ...
  const isPokharaActivity = (activityName) => {
    ...
    const pokharaKeywords = [
      ...
      'world peace pagoda',
      // Add all Pokhara activities from asset folders
      'biking', 'boating', 'bungee jumping', 'trekking', 'ziplining', 'tal barahi',
      'mahendra cave', 'bindyabasini', 'annapurna', 'base camp'
    ];
  ...
  // Categorize selected activities by city using the city property passed from App.jsx
  const selectedKathmanduActivities = selectedActivities
    .filter(activity => {
      // Use city property if available, fallback to keyword detection
      const isKathmandu = activity.city === 'kathmandu' || isKathmanduActivity(activity.name);
      console.log(`🏛️ ${activity.name} is Kathmandu activity: ${isKathmandu} (city: ${activity.city})`);
      return isKathmandu;
    })
    .map(activity => {
  ...
  const selectedPokharaActivities = selectedActivities
    .filter(activity => {
      // Use city property if available, fallback to keyword detection
      const isPokhara = activity.city === 'pokhara' || isPokharaActivity(activity.name);
      console.log(`🏔️ ${activity.name} is Pokhara activity: ${isPokhara} (city: ${activity.city})`);
      return isPokhara;
    })
    .map(activity => {
      // Try different possible price properties
